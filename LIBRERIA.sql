...
-- 1 emitir un listado con los clientes y los vendedores
-- primero los vendedores y luego los clientes ordenado
-- ordenado alfabeticamenre por apellido y nombre

select cod_cliente 'Codigo', ape_cliente 'Apellido', nom_cliente 'Nombre', 'Cliente' Tipo
from clientes
union 
select cod_vendedor 'Codigo', ape_vendedor 'Apellido', nom_vendedor 'Nombre', 'Vendedor' Tipo
from vendedores
order by 4 desc, 2, 3
-- no es necesario colocar el alias en ambos select
-- RESTRINCIONES == misma cantidad de columnas, mismo tipo de dato y el order by al final
...
select cod_cliente Codigo, ape_cliente + ' ' + nom_cliente Nombre, 'Cliente' Tipo, nro_tel, [e-mail]
from clientes
where nro_tel is not null and [e-mail] is not null
union all
select cod_vendedor Codigo, ape_vendedor + ' ' + nom_vendedor, 'Vendedor' Tipo, nro_tel, [e-mail]
from vendedores
where nro_tel is not null and [e-mail] is not null
order by 3, 2
...
select cod_cliente 'Codigo', nom_cliente + ' ' + ape_cliente as 'Nombre completo', 'Cliente' Tipo
from clientes
union 
select cod_vendedor 'Codigo', nom_vendedor + ' ' + ape_vendedor, 'Vendedor' Tipo
from vendedores
union 
...
select nom_cliente + ' ' + ape_cliente 'NOMBRE', c.calle + ' ' + str(c.altura) 'DIRECCION' , b.barrio 'BARRIO', 'Cliente' as 'INTEGRANTE'
from clientes as c join barrios as b on b.cod_barrio = c.cod_barrio
where c.calle is not null
union
select nom_vendedor + ' ' + ape_vendedor, v.calle + ' ' + str(v.altura) 'DIRECCION', b.barrio, 'Vendedor' Integrante
from vendedores as v join barrios as b on v.cod_barrio=b.cod_barrio
where v.cod_barrio between 3 and 12 and v.calle is not null
order by 1, 4
...
select descripcion 'Nombre Completo', 'Articulo' Tipo, a.pre_unitario 'Precio Articulo'
from articulos as a
where pre_unitario between 10 and 100
union
select nom_cliente + ' ' + ape_cliente 'Nombre', 'Cliente' Tipo, df.pre_unitario 
...
where ape_cliente like 'm%' or ape_cliente like 'p%'
...
select sum(df.pre_unitario * cantidad) 'Facturacion', 'Año Pasado' Fecha
from facturas as f join detalle_facturas as df on f.nro_factura = df.nro_factura
where year(fecha) = year(getdate())-1
union
select sum(df.pre_unitario * cantidad) 'Facturacion', 'Mes Pasado' Fecha
from facturas as f join detalle_facturas as df on f.nro_factura = df.nro_factura
where month(fecha) = month(getdate())-1 and year(fecha) = year(getdate())
union
select sum(df.pre_unitario * cantidad) 'Facturacion', 'Mes Actual' Fecha
from facturas as f join detalle_facturas as df on f.nro_factura = df.nro_factura
where month(fecha) = month(getdate()) and year(fecha) = year(getdate())
union
select sum(df.pre_unitario * cantidad) 'Facturacion', 'Hoy' Fecha
from facturas as f join detalle_facturas as df on f.nro_factura = df.nro_factura
where fecha = getdate()


-- CONSULTAS SUMARIAS: Funciones de agregado
-- cuantas facturas se hicieron en este negocio en el 2020 ?
...
-- cuanto facturo este negocio en total ?
select SUM(cantidad*pre_unitario) 'Importe total'
from detalle_facturas

-- cuanto facturo este negocio en anio pasado ?
select SUM(cantidad*pre_unitario) 'Importe total', count(*) 'Cantidad de detalles del anio pasado',
count(distinct df.nro_factura) 'cantidad de facturas del anio pasado',
...
from detalle_facturas as df join facturas as f on f.nro_factura = df.nro_factura
where year(fecha) = YEAR(getdate())-1

-- facturo el anio pasado 
select * from facturas
where year(fecha) = year(getdate())-1

-- mostrar el precio mas caro, el mas barato y el promedio de precios
-- de toda la facturacion

select max(pre_unitario) 'mas caro', min(pre_unitario) 'mas barato', avg(pre_unitario) 'promedio',
count(*) 'Cantidad de registros'
from detalle_facturas as df

select count(*) 'cantidad de registros', count(cod_cliente) 'cantidad de reg. donde cod_cliente no es nulo',
count([e-mail]) 'cantidad de clientes con email conocido'
from clientes
...
select sum(pre_unitario*cantidad) 'Total Facturado', sum(cantidad) 'Cantidad de articulos vendidos',
count(*) 'Cantidad de ventas', max(pre_unitario) 'Precio Max', min(pre_unitario) 'Precio min'
from detalle_facturas
where nro_factura = 236

--3. Se nos solicita además lo siguiente: ¿Cuánto se facturó el año pasado?

select sum(df.pre_unitario*df.cantidad) 'Facturacion del año pasado'
from facturas as f join detalle_facturas as df on f.nro_factura = df.nro_factura
where year(f.fecha) = year(getdate())-1

-- 4. ¿Cantidad de clientes con dirección de e-mail sea conocido (no nulo)
select count(*) 'Cantidad de clientes', count([e-mail]) 'Cantidad de clientes con email no nulo'
from clientes as c
...
select sum(df.pre_unitario*cantidad) 'Facturacion Total', count(distinct f.nro_factura) 'Cant. Facturas'
from facturas as f join detalle_facturas as df on f.nro_factura = df.nro_factura

--6. Se necesita conocer el promedio de monto facturado por factura el año 
--pasado.

select avg(pre_unitario*cantidad) 'Promedio por detalle Factura', sum(pre_unitario*cantidad)/count(distinct f.nro_factura)
'Promedio por Factura'
from facturas as f join detalle_facturas as df on f.nro_factura = df.nro_factura

-- 7. Se quiere saber la cantidad de ventas que hizo el vendedor de código 3.

select count(f.nro_factura) 'Cantidad de ventas'
...
select max(fecha) 'Fecha mas nueva', min(fecha) 'Fecha mas vieja'
from facturas
...
select count(f.nro_factura) 'Cant. vendidas', sum(cantidad) 'Cant. Art. Vendidos',
...
select count(distinct f.nro_factura) 'Cant. unidades vendidas', sum(pre_unitario*cantidad) 'Importe total',
sum(pre_unitario*cantidad)/count(distinct f.nro_factura) 'Importe promedio'
from vendedores as v join facturas as f on v.cod_vendedor = f.cod_vendedor
join detalle_facturas as df on df.nro_factura = f.nro_factura
...
select sum(cantidad*pre_unitario) 'Importe total Vendido', avg(cantidad*pre_unitario) 'Promedio',
sum(cantidad) 'cant. Articulos vendidos'
from facturas as f join detalle_facturas as df on f.nro_factura = df.nro_factura
...
select min(fecha) 'Primera Venta', sum(cantidad) 'Cant. Total vendida',
...
select sum(cantidad) 'Canti. articulos vendidos', sum(pre_unitario*cantidad)'Importe total'
from facturas as f join detalle_facturas as df on f.nro_factura = df.nro_factura
where fecha between '02/12/2011' and '15/06/2017'

--14.Se quiere saber la cantidad de veces y la última vez que vino el cliente de 
--apellido Abarca y cuánto gastó en total.

select count(distinct f.nro_factura)'Cant. Veces', sum(cantidad*pre_unitario) 'Gasto total', max(fecha) 'Ultima vez que vino'
from facturas as f join detalle_facturas as df on f.nro_factura = df.nro_factura
join clientes as c on c.cod_cliente = f.cod_cliente
...
count([e-mail]) 'Cant. cliente con direccion conocida'
from facturas as f join detalle_facturas as df on f.nro_factura = df.nro_factura
join clientes as c on c.cod_cliente = f.cod_cliente
...
select sum(cantidad*pre_unitario) 'Importe total Vend.', avg(cantidad*pre_unitario) 'Promedio vendido'
from facturas as f join detalle_facturas as df on f.nro_factura = df.nro_factura
